...
HabboView.add(function () {
    if (document.habboLoggedIn && $("subnavi-user")) {
        var b = new QuickMenu();
        var a = $A([
            ["myfriends", "test/quickmenu/friends_all"],
...
PersonalInfo.init = function (f) {
    ...
    Event.observe("feed-items", "click", Event.delegate({
        "li.contributed .remove-feed-item": function (n) {
            ...
            l.remove();
            new Ajax.Request("/habblet/ajax/removeFeedItem", {
                parameters: {
                    feedItemIndex: m
                }
            })
...
var GroupPurchase = function () {
    ...
    return {
        open: function () {
            a = Dialog.createDialog("group_purchase_form", L10N.get("Créer un groupe"), 9001, 0, -1000, GroupPurchase.close);
            Dialog.setAsWaitDialog(a);
